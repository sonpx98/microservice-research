...
import { ReadingService, GeneratedReading } from './reading.service';
    ...
    async handleCron() {
        ...
        for (const level of this.levels) {
            ...
            try {
                ...
                // Save directly (Bull queue removed)
                await this.readingService.save(readingPayload);

                this.logger.log(`Saved reading for ${level}: ${generatedData.title}`);

            } catch (error) {
                this.logger.error(`Failed to generate reading for ${level}`, error);
            }
        }
    }
}
